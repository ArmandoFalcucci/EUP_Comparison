...
| El                    | El                     | Elongation                                                 | Elongation           | artefact lenght/artefact width                                                                                         |
...
mutate(Torsion = recode(Torsion, "Twisted" = "Yes", "Straight"= "No", "Curved"= "No", "Very Curved"="No","Very curved"="No", "Slightly Curved"="No", "Slightly curved"="No",
 "ND"= "NA"))%>%
...
mutate(Profile = case_when(
  ...
  mutate(Distal.end.morpho= case_when(
    Distal.end.morpho %in% c("No Dist Part","Stepped") ~ NA, #stepped is assimilated to not complete
    Distal.end.morpho %in% c("?", "Hinged") ~ "ND", #assimilated to undetermined as it's an accident and not the real termination
...
mutate(Torsion = Profile, .after = Profile) %>% #INSERT Torsion
mutate(Torsion = recode(Torsion, "Twisted" = "Yes", "Straight"= "No", "Curved"= "No", "Very curved"="No", "Slightly curved"="No", "ND"= "NA"))%>%
  mutate(Torsion= case_when(
    Torsion == "NA" ~ NA,
  TRUE ~ Torsion
  )) %>% #Torsion
...
```{r}
Blank_Main_merged <- rbind(Gennai_ROM_ANS_Blanks_reshaped, Falcucci_FUM_Blanks_reshaped, Niochet_CTS_Blanks_reshaped)
library(writexl)
writexl::write_xlsx(Blank_Main_merged, "Ans_Rom_Fum_CTS_Blanks_merged.xlsx")
```
