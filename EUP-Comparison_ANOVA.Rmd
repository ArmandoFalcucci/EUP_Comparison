...
ggsave("ANOVAMCAcombinedPlat2705.tiff", plot = ANOVAPlatformMCA2, width = 66.96604, height = 32.75542, units = "cm", dpi = 300) 
ggsave("ANOVAMCAcombinedPlat2705.png", plot = ANOVAPlatformMCA2, width = 66.96604, height = 32.75542, units = "cm", dpi = 300)
...
SiteBlank <- as.data.frame(convexity_maintenance_ALL2$SiteBlank.type2)
...
Mca.Convexity2 <- mca_Convexity_fin2 %>%
  mutate(SiteBlank = convexity_maintenance_ALL2$SiteBlank.type2, "dim1" = coord.Dim.1, "dim2" =  coord.Dim.2)%>%
  select(4:6) #creating a dataframe with the Dim1 and Dim2 coordinates obtained with MCA_Reduction of each individual (blank)
...
ggsave("ANOVAMCAcombinedConv2705.tiff", plot = ANOVAConvexityMCA2, width = 66.96604, height = 32.75542, units = "cm", dpi = 300) 
ggsave("ANOVAMCAcombinedConv2705.png", plot = ANOVAConvexityMCA2, width = 66.96604, height = 32.75542, units = "cm", dpi = 300)
...
```{r}
library(dplyr)
MCA_DimConvexity <- as.data.frame(res.mca_DimConvexity$ind)
SiteBlank <- as.data.frame(dimconvexity_maintenance$SiteBlank.type2)

mca.DimConvexitydim1_2 <- MCA_DimConvexity %>%
  select(1:2)

mca_DimConvexity_fin2 <- bind_cols(SiteBlank, mca.DimConvexitydim1_2)

Mca.DimConvexity <- mca_DimConvexity_fin2 %>%
  mutate(SiteBlank = dimconvexity_maintenance$SiteBlank.type2, "dim1" = coord.Dim.1, "dim2" =  coord.Dim.2)%>%
  select(4:6) #creating a dataframe with the Dim1 and Dim2 coordinates obtained with MCA_Reduction of each individual (blank)

###create unique comparisons
unique(Mca.DimConvexity$SiteBlank)
...
boot_dim1_2 <- mcppb20(dim1 ~ SiteBlank, data = Mca.DimConvexity, nboot = 5000)
boot_dim2_2 <- mcppb20(dim2 ~ SiteBlank, data = Mca.DimConvexity, nboot = 5000)
...
# Add SiteType names and transform to column "SiteType"
multi_comp_dim1_2$SiteBlank <- comparisons_DimConvexity
multi_comp_dim1_2 <- multi_comp_dim1_2[!duplicated(names(multi_comp_dim1_2))]

multi_comp_dim2_2$SiteBlank <- comparisons_DimConvexity
multi_comp_dim2_2 <- multi_comp_dim2_2[!duplicated(names(multi_comp_dim2_2))]
...
```
##Plotting
When you see "abd" in this context, it's not a combination of "a", "b", and "d" in sequence. Rather, it's an indication that the group shares similarity with other groups having each of these individual letters "a", "b", and "d". So, it's interpreted as the presence of "a", "b", and "d" rather than a specific sequence. Each letter represents a characteristic that the group shares with other groups assigned the same letter in the same dimension.
...
# Arrange plots vertically
ANOVADimConvexity <- grid.arrange(dim1_plot, dim2_plot, ncol = 1)

# Save plots as TIFF and PNG files
ggsave("ANOVADimConvexity.tiff", plot = ANOVADimConvexity, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
ggsave("ANOVADimConvexity.png", plot = ANOVADimConvexity, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)

ANOVADimConvexityMCA <- grid.arrange(ANOVADimConvexity, MCA_DimConvexity_plot, ncol = 1)

ggsave("ANOVAMCAcombinedDimConv2705.tiff", plot = ANOVADimConvexityMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300) 
ggsave("ANOVAMCAcombinedDimConv2705.png", plot = ANOVADimConvexityMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300)
```
#Retouching
#blades and bladelets distinct
##preparing the data
...
ggsave("ANOVAMCAcombinedRetouching.tiff", plot = ANOVARetouchingMCA2, width = 66.96604, height = 32.75542, units = "cm", dpi = 300) 
ggsave("ANOVAMCAcombinedRetouching.png", plot = ANOVARetouchingMCA2, width = 66.96604, height = 32.75542, units = "cm", dpi = 300)
