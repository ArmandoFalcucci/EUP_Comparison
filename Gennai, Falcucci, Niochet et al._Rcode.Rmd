...
Niochet_Blanks_reshaped <- Niochet_CTS_US04inf_Blanks %>%
  ...
  filter(RawMatSimpl == "RMLocal") %>% #only local raw material
  mutate(Cortex.y.n = Cortex, .after = Cortex.position) %>% #INSERT CORTEX.Y.N
  mutate(Cortex.y.n = recode(Cortex.y.n, Extensive = "Yes", Full = "Yes", Semi ="Yes"))%>%
  mutate(Torsion = Profile, .after = Profile) %>% #INSERT Torsion
  mutate(Torsion = recode(Torsion, Twisted = "Yes", Straight= "No", Curved= "No", "Very Curved"="No", "Slightly Curved"="No"))%>%
...
mutate(Lip.simpl = case_when(
  ...
  Outline.morphology == "Off-axis" | Axiality.simpl == "Off axis" ~ "Déjeté",
...
library(writexl)
write_xlsx(Blank_Main_reshaped1, "Ans_Rom_Fum_CTS_Blanksmerged_reshaped.xlsx")
...
ANBld <- Blank_Main_reshaped1 %>%
  filter(Site == "AN", Blank.type == "Blade") %>%
  mutate(Cortex.position.simpl_lump = Cortex.position.simpl, .after = Cortex.position.simpl) %>%
  mutate(Platform_lump = Platform, .after = Platform)

ANCxBld <- ANBld %>%
  filter(Site == "AN", Blank.type == "Blade", Cortex.y.n== "Yes")

# Check the proportions table
proportions_table <- table(ANCxBld$Cortex.position.simpl_lump, useNA = "no") / sum(!is.na(ANCxBld$Cortex.position.simpl_lump)) * 100
print(proportions_table)

ANPlatBld <- ANBld %>%
  filter(Site == "AN", Blank.type == "Blade", !Platform== "No Prox part") 

# Check the proportions table
proportions_table <- table(ANPlatBld$Platform_lump, useNA = "no") / sum(!is.na(ANPlatBld$Platform_lump)) * 100
print(proportions_table)

# Modify levels based on proportions and create "Other" category
ANBld <- ANBld %>%
  mutate(Platform_lump = case_when(
    Platform_lump %in% c("Concave", "Crushed", "Dihedral", "Natural", "ND") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Platform_lump))) %>%
...
ANBldl <- Blank_Main_reshaped1 %>% 
  filter(Site == "AN", Blank.type == "Bladelet") %>%
mutate(Cortex.position.simpl_lump = Cortex.position.simpl, .after = Cortex.position.simpl)%>%
  mutate(Platform_lump = Platform, .after = Platform)

ANCxBldl <- ANBldl %>%
  filter(Site == "AN", Blank.type == "Bladelet", Cortex.y.n== "Yes")

proportions_table <- table(ANCxBldl$Cortex.position.simpl_lump, useNA = "no") / sum(!is.na(ANCxBldl$Cortex.position.simpl_lump)) * 100
print(proportions_table)

ANPlatBldl <- ANBldl %>%
  filter(Site == "AN", Blank.type == "Bladelet", !Platform== "No Prox part") %>%
  mutate(Platform_lump = Platform, .after = Platform)

# Check the proportions table
proportions_table <- table(ANPlatBldl$Platform_lump, useNA = "no") / sum(!is.na(ANPlatBldl$Platform_lump)) * 100
print(proportions_table)

# Modify levels based on proportions and create "Other" category
ANBldl <- ANBldl %>%
  mutate(Platform_lump = case_when(
    Platform_lump %in% c("Distal+Proximal", "Proximal") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Platform_lump) 
  )) %>%
  mutate(Cortex.position.simpl_lump = case_when(
...
ANFl <- Blank_Main_reshaped1 %>% 
  filter(Site == "AN", Blank.type == "Flake")%>%
mutate(Cortex.position.simpl_lump = Cortex.position.simpl, .after = Cortex.position.simpl) %>%
mutate(Platform_lump = Platform, .after = Platform)

ANCxFl <- ANFl %>%
  filter(Site == "AN", Blank.type == "Flake", Cortex.y.n== "Yes")

proportions_table <- table(ANCxFl$Cortex.position.simpl_lump, useNA = "no") / sum(!is.na(ANCxFl$Cortex.position.simpl_lump)) * 100
print(proportions_table)

ANPlatFl <- ANFl %>%
  filter(Site == "AN", Blank.type == "Flake", !Platform== "No Prox part") %>%
  mutate(Platform_lump = Platform, .after = Platform)

# Check the proportions table
proportions_table <- table(ANPlatFl$Platform_lump, useNA = "no") / sum(!is.na(ANPlatFl$Platform_lump)) * 100
print(proportions_table)

# Modify levels based on proportions and create "Other" category
ANFl <- ANFl %>%
  mutate(Platform_lump = case_when(
    Platform_lump %in% c("Crushed", "Dihedral", "ND", "Punctiform") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Platform_lump)
  )) %>%
  mutate(Cortex.position.simpl_lump = case_when(
...
CTSBldl <- Blank_Main_reshaped1 %>% 
  filter(Site == "CTS", Blank.type == "Bladelet")%>%
mutate(Cortex.position.simpl_lump = Cortex.position.simpl, .after = Cortex.position.simpl)%>%
mutate(Platform_lump = Platform, .after = Platform)

CTSCxBldl <- CTSBldl %>%
  filter(Site == "CTS", Blank.type == "Bladelet", Cortex.y.n== "Yes")

proportions_table <- table(CTSCxBldl$Cortex.position.simpl_lump, useNA = "no") / sum(!is.na(CTSCxBldl$Cortex.position.simpl_lump)) * 100
print(proportions_table)

CTSPlatBldl <- CTSBldl %>%
  filter(Site == "CTS", Blank.type == "Bladelet", !Platform== "No butt")

# Check the proportions table
proportions_table <- table(CTSPlatBldl$Platform_lump, useNA = "no") / sum(!is.na(CTSPlatBldl$Platform_lump)) * 100
print(proportions_table)

# Modify levels based on proportions and create "Other" category
CTSBldl <- CTSBldl %>%
  mutate(Platform_lump = case_when(
    Platform_lump %in% c("Concave", "Dihedral", "Faceted", "Natural") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Platform_lump)
  ))%>%
  mutate(Cortex.position.simpl_lump = case_when(
    Cortex.position.simpl_lump %in% c("Bilateral", "Lateral+Dorsal", "Lateral+Proximal", "ND", "Proximal") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Cortex.position.simpl_lump)  
  ))

CTSBld <- Blank_Main_reshaped1 %>% 
  filter(Site == "CTS", Blank.type == "Blade") %>%
mutate(Cortex.position.simpl_lump = Cortex.position.simpl, .after = Cortex.position.simpl) %>%
  mutate(Platform_lump = Platform, .after = Platform)

CTSCxBld <- CTSBld %>%
  filter(Site == "CTS", Blank.type == "Blade", Cortex.y.n== "Yes")

proportions_table <- table(CTSCxBld$Cortex.position.simpl_lump, useNA = "no") / sum(!is.na(CTSCxBld$Cortex.position.simpl_lump)) * 100
print(proportions_table)

CTSPlatBld <- CTSBld %>%
  filter(Site == "CTS", Blank.type == "Blade", !Platform== "No butt") 

# Check the proportions table
proportions_table <- table(CTSPlatBld$Platform_lump, useNA = "no") / sum(!is.na(CTSPlatBld$Platform_lump)) * 100
print(proportions_table)

# Modify levels based on proportions and create "Other" category
CTSBld <- CTSBld %>%
  mutate(Platform_lump = case_when(
    Platform_lump %in% c("Concave", "Dihedral", "Natural", "ND", "Punctiform") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Platform_lump)
  ))%>%
  mutate(Cortex.position.simpl_lump = case_when(
    Cortex.position.simpl_lump %in% c("Bilateral", "Lateral+Distal", "Lateral+Dorsal", "Lateral+Proximal") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Cortex.position.simpl_lump)  
  ))

CTSPlatFl <- Blank_Main_reshaped1 %>%
  filter(Site == "CTS", Blank.type == "Flake", !Platform== "No butt") %>%
  mutate(Platform_lump = Platform, .after = Platform)

# Check the proportions table
proportions_table <- table(CTSPlatFl$Platform_lump, useNA = "no") / sum(!is.na(CTSPlatFl$Platform_lump)) * 100
print(proportions_table)

#no less than 5% categories
  
FUMBldl <- Blank_Main_reshaped1 %>% 
  filter(Site == "FUM", Blank.type == "Bladelet") %>%
mutate(Cortex.position.simpl_lump = Cortex.position.simpl, .after = Cortex.position.simpl)  %>%
  mutate(Platform_lump = Platform, .after = Platform)

FUMCxBldl <- FUMBldl %>%
  filter(Site == "FUM", Blank.type == "Bladelet", Cortex.y.n== "Yes")

proportions_table <- table(FUMCxBldl$Cortex.position.simpl_lump, useNA = "no") / sum(!is.na(FUMCxBldl$Cortex.position.simpl_lump)) * 100
print(proportions_table)

FUMPlatBldl <- FUMBldl %>%
  filter(Site == "FUM", Blank.type == "Bladelet", !Platform == "No butt")

# Check the proportions table
proportions_table <- table(FUMPlatBldl$Platform_lump, useNA = "no") / sum(!is.na(FUMPlatBldl$Platform_lump)) * 100
print(proportions_table)

# Modify levels based on proportions and create "Other" category
FUMBldl <- FUMBldl %>%
  mutate(Platform_lump = case_when(
    Platform_lump %in% c( "Dihedral", "Faceted", "ND") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Platform_lump)
  ))%>%
  mutate(Cortex.position.simpl_lump = case_when(
    Cortex.position.simpl_lump %in% c("Distal+Proximal", "Proximal") ~ "Other", #less than 5% categories
...
FUMBld <- Blank_Main_reshaped1 %>% 
  filter(Site == "FUM", Blank.type == "Blade") %>%
mutate(Cortex.position.simpl_lump = Cortex.position.simpl, .after = Cortex.position.simpl)  %>%
  mutate(Platform_lump = Platform, .after = Platform)

FUMCxBld <- FUMBld %>%
  filter(Site == "FUM", Blank.type == "Blade", Cortex.y.n== "Yes")

proportions_table <- table(FUMCxBld$Cortex.position.simpl_lump, useNA = "no") / sum(!is.na(FUMCxBld$Cortex.position.simpl_lump)) * 100
print(proportions_table)

FUMPlatBld<- FUMBld %>%
  filter(Site == "FUM", Blank.type == "Blade", !Platform== "No butt")

# Check the proportions table
proportions_table <- table(FUMPlatBld$Platform_lump, useNA = "no") / sum(!is.na(FUMPlatBld$Platform_lump)) * 100
print(proportions_table)

# Modify levels based on proportions and create "Other" category
FUMBld <- FUMBld %>%
  mutate(Platform_lump = case_when(
    Platform_lump %in% c( "Dihedral", "Faceted", "Natural", "Punctiform") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Platform_lump)
  )) %>%
  mutate(Cortex.position.simpl_lump = case_when(
...
FUMFl <- Blank_Main_reshaped1 %>% 
  filter(Site == "FUM", Blank.type == "Flake") %>%
mutate(Cortex.position.simpl_lump = Cortex.position.simpl, .after = Cortex.position.simpl) %>%
  mutate(Platform_lump = Platform, .after = Platform) 

FUMFl <- FUMFl %>%
  filter(Site == "FUM", Blank.type == "Flake", Cortex.y.n== "Yes")

proportions_table <- table(FUMCxBldl$Cortex.position.simpl_lump, useNA = "no") / sum(!is.na(FUMCxBldl$Cortex.position.simpl_lump)) * 100
print(proportions_table)

FUMPlatFl <- FUMFl %>%
  filter(Site == "FUM", Blank.type == "Flake", !Platform== "No butt") 
# Check the proportions table
proportions_table <- table(FUMPlatFl$Platform_lump, useNA = "no") / sum(!is.na(FUMPlatFl$Platform_lump)) * 100
print(proportions_table)

FUMFl <- FUMFl %>%
  mutate(Platform_lump = case_when(
    Platform_lump %in% c( "Dihedral", "Natural", "Punctiform") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Platform_lump)
  ))%>%
  mutate(Cortex.position.simpl_lump = case_when(
    Cortex.position.simpl_lump %in% c("Distal+Proximal") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Cortex.position.simpl_lump)  
  ))

ROMBldl <- Blank_Main_reshaped1 %>% 
  filter(Site == "ROM", Blank.type == "Bladelet") %>%
mutate(Cortex.position.simpl_lump = Cortex.position.simpl, .after = Cortex.position.simpl)%>%
  mutate(Platform_lump = Platform, .after = Platform)

ROMCxBldl <- ROMBldl %>%
  filter(Site == "ROM", Blank.type == "Bladelet", Cortex.y.n== "Yes")

proportions_table <- table(ROMCxBldl$Cortex.position.simpl_lump, useNA = "no") / sum(!is.na(ROMCxBldl$Cortex.position.simpl_lump)) * 100
print(proportions_table)

ROMPlatBldl <- ROMBldl %>%
  filter(Site == "ROM", Blank.type == "Bladelet", !Platform== "No Prox part") 

# Check the proportions table
proportions_table <- table(ROMPlatBldl$Platform_lump, useNA = "no") / sum(!is.na(ROMPlatBldl$Platform_lump)) * 100
print(proportions_table)

# Modify levels based on proportions and create "Other" category
ROMBldl <- ROMBldl %>%
  mutate(Platform_lump = case_when(
    Platform_lump %in% c("Dihedral", "Natural", "ND") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Platform_lump)
  ))

ROMBld <- Blank_Main_reshaped1 %>% 
  filter(Site == "ROM", Blank.type == "Blade")%>%
mutate(Cortex.position.simpl_lump = Cortex.position.simpl, .after = Cortex.position.simpl)%>%
  mutate(Platform_lump = Platform, .after = Platform) 

ROMCxBld <- ROMBld %>%
  filter(Site == "ROM", Blank.type == "Blade", Cortex.y.n== "Yes")

proportions_table <- table(ROMCxBld$Cortex.position.simpl_lump, useNA = "no") / sum(!is.na(ROMCxBld$Cortex.position.simpl_lump)) * 100
print(proportions_table)

ROMPlatBld <- ROMBld %>%
  filter(Site == "ROM", Blank.type == "Blade", !Platform== "No Prox part") 

# Check the proportions table
proportions_table <- table(ROMPlatBld$Platform_lump, useNA = "no") / sum(!is.na(ROMPlatBld$Platform_lump)) * 100
print(proportions_table)

# Modify levels based on proportions and create "Other" category
ROMBld <- ROMBld %>%
  mutate(Platform_lump = case_when(
    Platform_lump %in% c("Crushed", "Dihedral", "Faceted", "ND") ~ "Other", #less than 5% categories
    TRUE ~ as.character(Platform_lump)
    ))%>%
  mutate(Cortex.position.simpl_lump = case_when(
...
ROMFl <- Blank_Main_reshaped1 %>% 
  filter(Site == "ROM", Blank.type == "Flake")%>%
mutate(Cortex.position.simpl_lump = Cortex.position.simpl, .after = Cortex.position.simpl) %>%
  mutate(Platform_lump = Platform, .after = Platform)

ROMCxFl <- ROMFl %>%
  filter(Site == "ROM", Blank.type == "Flake", Cortex.y.n== "Yes")

proportions_table <- table(ROMCxFl$Cortex.position.simpl_lump, useNA = "no") / sum(!is.na(ROMCxFl$Cortex.position.simpl_lump)) * 100
print(proportions_table)

ROMPlatFl <- ROMFl %>%
  filter(Site == "ROM", Blank.type == "Flake", !Platform== "No Prox part") 

# Check the proportions table
proportions_table <- table(ROMPlatFl$Platform_lump, useNA = "no") / sum(!is.na(ROMPlatFl$Platform_lump)) * 100
print(proportions_table)
...
library(writexl)
writexl::write_xlsx(Blanks_Main_lumped, "Ans_Rom_Fum_CTS_Blanks_lumped.xlsx")
```
...
library(readxl)
library(dplyr)
Blanks_Main_lumped <- read_excel(file.choose()) #get the Ans_Rom_Fum_CTS_Blanks_lumped.xlsx
```
...
ggsave("BlankTechPhase.png", plot = Blank.Tech.Phase, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Init.Cortex.png", plot = Init.Cortex, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("MGMT.Cortex.png", plot = MGMT.Cortex, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Ordinary.Cortex.png", plot = Ordinary.Cortex, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
###cortical position
```{r}
unique(Blanks_Main_lumped$Cortex.position.simpl_lump)
Blanks_Main_lumped$Cortex.position.simpl_lump_F <- factor(Blanks_Main_lumped$Cortex.position.simpl_lump, levels= c("Proximal", "Lateral", "Lateral+Proximal", "Lateral+Distal", "Distal", "Dorsal", "Other")) #proximal to distal
...
Gen.Cortex.Pos <- grouped_ggbarstats(
  data = filter(Blanks_Main_lumped, Cortex.y.n == "Yes"), 
...
Init.Cortex.Pos <- grouped_ggbarstats(
  data = filter(Blanks_Main_lumped, Cortex.y.n == "Yes", Technology.Phase_F == "Init" ), 
  x = Cortex.position.simpl_lump_F,  
  y = Site_F, 
  grouping.var = Blank.type_F,
  results.subtitle = FALSE,
...
ggsave("Init.Cortex.Pos.png", plot = Init.Cortex.Pos, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("MGMT.Cortex.Pos.png", plot = MGMT.Cortex.Pos, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Ordinary.Cortex.Pos.png", plot = Ordinary.Cortex.Pos, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```

###platform

```{r}
unique(Blanks_Main_lumped$Platform.simpl)
Blanks_Main_lumped$Platform.simpl_F <- factor(Blanks_Main_lumped$Platform.simpl, levels= c("Plain", "ND" ,"Natural","Faceted" ,"Crushed")) #unprepared to prepared
...
Gen.Platform.simpl <- grouped_ggbarstats(
  data = filter(Blanks_Main_lumped, Preservation !="Mes+Dist", !Platform.simpl == "ND"), #only blanks with a platform
  x = Platform.simpl_F,  
  y = Site_F, 
  grouping.var = Blank.type_F,
  xlab="Site",
...
```{r}
library(dplyr)
unique(Blanks_Main_lumped$Platform_lump)
Blanks_Main_lumped$Platform_lump_F <- factor(Blanks_Main_lumped$Platform_lump, levels= c("Linear","Other" , "Plain" , "Punctiform", "ND", "Natural" , "Crushed", "Faceted")) #unprepared to prepared


library(RColorBrewer)
# Get unique values in the Platform.Simpl column
platform_values <- unique(Blanks_Main_lumped$Platform_lump_F)
...
Gen.Platform <- grouped_ggbarstats(
  data = filter(Blanks_Main_lumped, Preservation !="Mes+Dist", !Platform_lump_F == "ND"), 
  x = Platform_lump_F,  
  y = Site_F, 
  xlab="Site",
  grouping.var = Blank.type_F,
...
ggsave("Gen.Platformlump.tiff", plot = Gen.Platform, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
ggsave("Gen.Platformlump.png", plot = Gen.Platform, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
...
Init.Platform <- grouped_ggbarstats(
  data = filter(Blanks_Main_lumped, Preservation !="Mes+Dist", !Platform_lump_F == "ND",  Technology.Phase_F == "Init"), 
  x = Platform_lump_F, 
  y = Site_F, 
  grouping.var = Blank.type_F,
  xlab="Site",
...
ggsave("Init.Platform.png", plot = Init.Platform, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```

```{r}
MGMT.Platform <- grouped_ggbarstats(
  data = filter(Blanks_Main_lumped, Preservation !="Mes+Dist", !Platform_lump_F == "ND",  Technology.Phase_F == "MGMT"), 
  x = Platform_lump_F,  
  y = Site_F, 
  xlab="Site",
  grouping.var = Blank.type_F,
...
ggsave("MGMT.Platform.png", plot = MGMT.Platform, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```

```{r}
Ordinary.Platform <- grouped_ggbarstats(
  data = filter(Blanks_Main_lumped, Preservation !="Mes+Dist", !Platform_lump_F == "ND",  Technology.Phase_F == "Ordinary"), 
  x = Platform_lump_F,   
  y = Site_F, 
  grouping.var = Blank.type_F,
  xlab="Site",
...
ggsave("ordinary.Platform.png", plot = Ordinary.Platform, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Init.Abrasion.png", plot = Init.Abrasion, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("MGMT.Abrasion.png", plot = MGMT.Abrasion, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Ordinary.Abrasion.png", plot = Ordinary.Abrasion, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Init.Axiality.png", plot = Init.Ax, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("MGMT.Axiality.png", plot = MGMT.Ax, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Ordinary.Axiality.png", plot = Ordinary.Ax, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()

```

```{r}
Blanks_Main_lumped$Outline.morphology_F <- factor(Blanks_Main_lumped$Outline.morphology, levels=c("Subparallel edges", "Convergent", "Déjeté", "ND", "Other")) 
library(RColorBrewer)
# Get unique values in the Platform.Simpl column
...
ggsave("Init.Out.Morph.png", plot = Init.Out.Morph, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("MGMT.Out.Morph.png", plot = MGMT.Out.Morph, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Ordinary.Out.Morph.png", plot = Ordinary.Out.Morph, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Init.Symmetry.png", plot = Init.Symmetry, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("MGMT.Symmetry.png", plot = MGMT.Symmetry, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Ordinary.Symmetry.png", plot = Ordinary.Symmetry, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
###crosssection shape
```{r}
...
ggsave("Init.Cross.Section.png", plot = Init.Cross.Section, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("MGMT.Cross.Section.png", plot = MGMT.Cross.Section, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Ordinary.Cross.Section.png", plot = Ordinary.Cross.Section, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
###profile
```{r}
...
ggsave("Init.Profile.png", plot = Init.Profile, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("MGMT.Profile.png", plot = MGMT.Profile, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Ordinary.Profile.png", plot = Ordinary.Profile, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Init.Distal.end.morpho.png", plot = Init.Distal.end.morpho, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("MGMT.Distal.end.morpho.png", plot = MGMT.Distal.end.morpho, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
ggsave("Ordinary.Distal.end.morpho.png", plot = Ordinary.Distal.end.morpho, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
###torsion
```{r}
library(RColorBrewer)
...
###dorsal scar pattern
```{r}
unique(Blanks_Main_lumped$Dorsal.scar.1)
...
Gen.Dorsal.scar.1 <- grouped_ggbarstats(
  data = filter(Blanks_Main_lumped, Dorsal.scar.1_F != "ND", Dorsal.scar.1_F != "No negatives"), 
  x = Dorsal.scar.1_F,  
  y = Site_F, 
  grouping.var = Blank.type_F,
  results.subtitle = FALSE,
...
Init.Dorsal.scar.1 <- grouped_ggbarstats(
  data = filter(Blanks_Main_lumped, Dorsal.scar.1_F != "ND", Dorsal.scar.1_F != "No negatives", Technology.Phase_F == "Init"),
  x = Dorsal.scar.1_F,  
  y = Site_F, 
  grouping.var = Blank.type_F,
  xlab="Site",
...
ggsave("Init.Dorsal.scar.1.png", plot = Init.Dorsal.scar.1, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
MGMT.Dorsal.scar.1 <- grouped_ggbarstats(
  data = filter(Blanks_Main_lumped, Dorsal.scar.1_F != "ND", Dorsal.scar.1_F != "No negatives",  Technology.Phase_F == "MGMT"), 
  x = Dorsal.scar.1_F,  
  y = Site_F, 
  grouping.var = Blank.type_F,
  xlab="Site", 
...
ggsave("MGMT.Dorsal.scar.1.png", plot = MGMT.Dorsal.scar.1, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
Ordinary.Dorsal.scar.1 <- grouped_ggbarstats(
  data = filter(Blanks_Main_lumped, Dorsal.scar.1_F != "ND", Dorsal.scar.1_F != "No negatives",  Technology.Phase_F == "Ordinary"), 
  x = Dorsal.scar.1_F,  
  y = Site_F, 
  grouping.var = Blank.type_F,
  xlab="Site",
...
ggsave("ordinary.Dorsal.scar.1.png", plot = Ordinary.Dorsal.scar.1, width = 27.49021, height = 20.13479, units = "cm", dpi = 300) #set your working directory with setwd()
```
...
library(readxl)
library(dplyr)
Blanks_Main_lumped <- read_excel(file.choose()) #get the Ans_Rom_Fum_CTS_Blankslumped.xlsx
View(Blanks_Main_lumped)
str(Blanks_Main_lumped)

```
## Plotting and analyising widths
```{r}
Blanks_Main_lumped$Site_F <- factor(Blanks_Main_lumped$Site, levels= c("AN", "ROM", "FUM", "CTS"))
Blanks_Main_lumped$Blank.type_F <- factor(Blanks_Main_lumped$Blank.type, levels= c("Flake", "Blade", "Bladelet"))
Blanks_Main_lumped$Technology.Phase_F <- factor(Blanks_Main_lumped$Technology.Phase, levels= c("Init", "MGMT", "Tablet", "Ordinary", "Spall")) #reorder earlier to later
```
...
```{r}
library(ggplot2)
Blanks_Main_lumped$Width <- as.numeric(Blanks_Main_lumped$Width)
Blanks_Main_lumped$Length <- as.numeric(Blanks_Main_lumped$Length)

# Filter the data and remove NAs from the 'Width' column
filtered_data <- filter(Blanks_Main_lumped, Blank.type_F %in% c("Blade", "Bladelet") & !is.na(Width))
filtered_data$Width <- as.numeric(filtered_data$Width)
# Create the plot
histowidthLaminar <- ggplot(filtered_data, aes(x = Width)) +
  ...
  scale_color_manual(values = "red", name = "Medians and Threshold", labels = "Blade and Bladelet Threshold")

histowidthLaminar
  

filtered_data_O <- filter(Blanks_Main_lumped, Blank.type_F %in% c("Blade", "Bladelet") & Technology.Phase_F == "Ordinary" & !is.na(Width))
filtered_data_O$Width <- as.numeric(filtered_data_O$Width)
# Create the plot
histowidthLaminarO <- ggplot(filtered_data_O, aes(x = Width)) +
  geom_histogram(aes(y = ..density..), color = "grey", fill = "grey", binwidth = 1) +
  geom_density() +
  facet_wrap(~Site_F, ncol = 2) +
  scale_y_continuous(labels = scales::percent) +
  geom_vline(xintercept = 12, linetype = "dashed", color = "red", size = 1) +
  scale_x_continuous(breaks = seq(0, max(filtered_data_O$Width, na.rm = TRUE), by = 2)) +
  theme(axis.title.y = element_blank(),
        axis.text.x = element_text(size = 4),
        legend.position = "top",
        legend.title = element_text(size = 6, face = "bold"),
...
median_blade <- Blanks_Main_lumped %>%
...
median_bladelet <- Blanks_Main_lumped %>%
...
# Group by site and calculate median width for Bladelet
median_bladeletO <- filtered_data_O %>%
...
# Group by site and calculate median width excluding Blade and Bladelet
median_laminarO <- filtered_data_O %>%
...
ggsave("histolaminarwidthO.png", plot = histolaminarwidthO, width = 15, height = 10, units = "cm", dpi = 300) #directory with setwd() 
...
Gen.width <- grouped_ggbetweenstats(
  data = filter(Blanks_Main_lumped, Blank.type_F %in% c("Blade", "Bladelet"), Tool == "No"),
  x = Site_F,
  y = Width,
  grouping.var = Blank.type_F,
  type = "non-parametric",
...
ggsave("Gen.width.png", plot = Gen.width, width = 27.49021 , height =20.29354, units = "cm", dpi = 300) #set your working directory with setwd()
```

```{r}
MGMT.width <- grouped_ggbetweenstats(
  data = filter(Blanks_Main_lumped, Blank.type_F %in% c("Blade", "Bladelet"), Tool == "No", Technology.Phase_F == "MGMT"),
  x = Site_F,
  y = Width,
  grouping.var = Blank.type_F,
  type = "non-parametric",
...
ggsave("MGMT.width.png", plot = MGMT.width, width = 27.49021 , height =20.29354, units = "cm", dpi = 300) #set your working directory with setwd()
```

```{r}
Ordinary.width <- grouped_ggbetweenstats(
  data = filter(Blanks_Main_lumped, Blank.type_F %in% c("Blade", "Bladelet"), Tool == "No", Technology.Phase_F == "Ordinary"),
  x = Site_F,
  y = Width,
  grouping.var = Blank.type_F,
  type = "non-parametric",
...
ggsave("Ordinary.width.png", plot = Ordinary.width,width = 27.49021 , height =20.29354, units = "cm", dpi = 300) #set your working directory with setwd()
```
the width are statistically significant across sites, but the effect size is weak or negligible.
####elongation
```{r}
Blanks_Main_lumped$Elongation <- as.numeric(Blanks_Main_lumped$Elongation)
Elongation <- Blanks_Main_lumped %>% filter(!is.na(Elongation))
Gen.elon <- grouped_ggbetweenstats(
  data = filter(Elongation, Preservation == "Complete", Tool == "No", Blank.type_F %in% c("Blade", "Bladelet")),
  x = Site_F,
  y = Elongation,
  grouping.var = Blank.type_F,
  type = "non-parametric",
...
ggsave("Gen.elon.png", plot = Gen.elon, width = 27.49021 , height =20.29354, units = "cm", dpi = 300) #set your working directory with setwd()
```


```{r}
MGMT.elon <- grouped_ggbetweenstats(
  data = filter(Elongation, Preservation == "Complete", Tool == "No", Blank.type_F %in% c("Blade", "Bladelet"), Technology.Phase_F == "MGMT"),
  x = Site_F,
  y = Elongation,
  grouping.var = Blank.type_F,
  type = "non-parametric",
...
ggsave("MGMT.elon.png", plot = MGMT.elon, width = 27.49021 , height =20.29354, units = "cm", dpi = 300) #set your working directory with setwd()
```


```{r}
Ordinary.elon <- grouped_ggbetweenstats(
  data = filter(Elongation, Preservation == "Complete", Tool == "No", Blank.type_F %in% c("Blade", "Bladelet"), Technology.Phase_F == "Ordinary"),
  x = Site_F,
  y = Elongation,
  grouping.var = Blank.type_F,
  type = "non-parametric",
...
ggsave("Ordinary.elon.png", plot = Ordinary.elon, width = 27.49021 , height =20.29354, units = "cm", dpi = 300) #set your working directory with setwd()
...
```{r}
library(dplyr)
Flaking.Int <- Blanks_Main_lumped %>%
...
ggsave("Flaking.Int.png", plot = Int, width = 15, height = 10, units = "cm", dpi = 300)
```
...
```{r}
library(dplyr)
Blanks_Main_lumped1 <- Blanks_Main_lumped %>%
...
str(Blanks_Main_lumped1)
library(writexl)
write_xlsx(Blanks_Main_lumped1, "Ans_Rom_Fum_CTS_Blanks_lumped1.xlsx")

```
## control group

We use Cascalheira et al's (2019) Soloutrean assemblages as control group

```{r}
library(readxl)
library(dplyr)
Sol <- read_excel(file.choose()) #get the Cascalheira_blanks.xlsx

#merge it with our DB
Sol_reshaped <- Sol %>%
  rename(Site="Context", Blank.type="Class", Cortex.position.simpl_lump="CortexLoc", Cortex.y.n="CortexPerc", Distal.end.morpho="Termination", Outline.morphology="EdgeShape", Profile="Profile", Cross.section="CrossSection", Number.negatives="ScarNumber", Platform_lump="PlatformType", Dorsal.scar.1= "DorsalPattern") %>%
  mutate(SiteBlank.type = Site, .after = Site) %>%
  mutate(SiteBlank.type = case_when(
    Blank.type == "Elongated product" & Width > 12.1 ~ "Control.B",
    Blank.type == "Elongated product" & Width < 12.1 ~ "Control.b",
    Blank.type == "Flake" ~ "Control.F"
  )) %>%
  mutate(Cortex.y.n = case_when(
    Cortex.y.n == 0 ~ "no",
    Cortex.y.n != 0 ~ "yes"
  ...
  mutate(Outline.morphology = case_when(
    Outline.morphology == "Parallel" ~ "Subparallel edges")) %>%
  mutate(Distal.end.morpho = case_when(
    Distal.end.morpho == "Overpassed"~ "Plunging",
    Distal.end.morpho == "Pointed"~ "Feathered")) %>%
  mutate(Cross.section = case_when(
    Cross.section == "Trapezoidal"~ "Polyhedric", 
   TRUE ~ Cross.section)
  )

 Blanks_Main_lumped2 <-  Blanks_Main_lumped1%>%
  select(Site, SiteBlank.type, Cortex.y.n,Length,Width,Thickness,Cortex.position.simpl_lump, Platform_lump,Outline.morphology,Cross.section,Profile,Distal.end.morpho,Blank.type,Number.negatives,Dorsal.scar.1)
 library(writexl)
write_xlsx(Blanks_Main_lumped2, "Ans_Rom_Fum_CTS_Blanks_lumped2.xlsx")
 
 Blank_Main_Control <- rbind(Blanks_Main_lumped2, Sol_reshaped)
 library(writexl)
write_xlsx(Blank_Main_Control, "Ans_Rom_Fum_CTS_Blanks_lumped&control.xlsx")
...
library(readxl)
library(dplyr)
Blank_Main_Control <- read_excel(file.choose()) #Ans_Rom_Fum_CTS_Blanks_lumped&control.xlsx
Blanks_Main_lumped1 <- read_excel(file.choose()) #Ans_Rom_Fum_CTS_Blanks_lumped1.xlsx

Blank_Main_Control_elon <- Blank_Main_Control %>%
  filter(Blank.type !="Flake")
write_xlsx(Blank_Main_Control_elon, "Ans_Rom_Fum_CTS_Blanks_lumped&control_ELON.xlsx")
```

What we want to check is the association of categorical variables in three domains of the knapping process across the site
...
The Platform domain is comprising the Variables Platform and Abrasion.simpl to check if there's variation in platform preparation during the knapping sequence.

```{r}
library(ggpubr)
PlatformDom <- ggarrange(Gen.Platform, Gen.Abrasion, labels = c("A", "B", "C"), ncol = 1, nrow =2)
...
platform_maintenanceGO <- Blanks_Main_lumped1 %>%
  filter(Blank.type != "Flake") %>% #check only laminar
  filter(Preservation != "Mes+Dist") %>% #only blanks with a Proximal part
  filter(!Platform_lump %in% c("No butt", "No Prox part", "ND", "NA", "Crushed", "Concave", "Natural", "Other")) %>% #removing blanks without clear info
...
fviz_contrib(res.mca_PlatformO, choice = "var", axes = 1, top = 15) #contribution to Dim 1. above the threshold are No, Other, Punctiform 

fviz_contrib(res.mca_PlatformO, choice = "var", axes = 2, top = 15) #contribution to Dim 2. above the threshold is only Plain, Punctiform, Linear

MCA_PlatformO_plot <- fviz_mca_var(
  res.mca_PlatformO,
  col.var = "contrib",
  label = "all",
  invisible = "none",
...
MCA_PlatformO_plot
ggsave("MCAPlatformOrdinaryNew.tiff", plot = MCA_PlatformO_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
ggsave("MCAPlatformOrdinaryNew.png", plot = MCA_PlatformO_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)    


MCA_PlatformO_ind <- fviz_mca_ind(
  res.mca_PlatformO, 
  label = "none",                   # hide individual labels
  habillage = "SiteBlank.type", # color by groups
  addEllipses = TRUE,               # add confidence ellipses
  ellipse.type = "confidence",      # type of ellipse
  ggtheme = theme_classic(),        # set the ggplot theme
  geom = "point",                   # use points for symbols
  ) # set custom colors              # use grouping variable for shapes

ggsave("MCAPlatformOrdinaryind.tiff", plot = MCA_PlatformO_ind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
ggsave("MCAPlatformOrdinaryind.png", plot = MCA_PlatformO_ind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
...
# Select relevant columns and remove missing values
platform_maintenanceGM <- Blanks_Main_lumped1 %>%
  filter(Blank.type != "Flake") %>% #check only laminar
  filter(Preservation != "Mes+Dist") %>% #only blanks with a Proximal part
  filter(!Platform_lump %in% c("No butt", "No Prox part", "ND", "NA", "Crushed", "Concave", "Natural", "Other")) %>% #removing blanks without clear info
  filter(!Abrasion.simpl %in% c("NA", "ND")) %>% #removing blanks without clear info
...
fviz_screeplot(res.mca_PlatformM, addlabels = TRUE, ylim = c(0, 45)) #To visualize the percentages of inertia explained by each MCA dimensions. Dim1 and Dim2 explain 61.034% of the variance
...
fviz_contrib(res.mca_PlatformM, choice = "var", axes = 1, top = 15) #contribution to Dim 1. above the threshold are  Abrasion.simpl_No, Natural

fviz_contrib(res.mca_PlatformM, choice = "var", axes = 2, top = 15) #contribution to Dim 2. above the threshold is other, Plain

MCA_PlatformM_plot <- fviz_mca_var(
  res.mca_PlatformM,
  col.var = "contrib",
  label = "all",
  invisible = "none",
...
MCA_PlatformM_plot
ggsave("MCAPlatformMGMTNew.tiff", plot = MCA_PlatformM_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
ggsave("MCAPlatformMGMTNew.png", plot = MCA_PlatformM_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)    


MCA_PlatformM_ind <- fviz_mca_ind(
  res.mca_PlatformM, 
  label = "none",                   # hide individual labels
  habillage = "SiteBlank.type", # color by groups
  addEllipses = TRUE,               # add confidence ellipses
...
ggsave("MCAPlatformMGMTind.tiff", plot = MCA_PlatformM_ind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
ggsave("MCAPlatformMGMTind.png", plot = MCA_PlatformM_ind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
...
# Select relevant columns and remove missing values
platform_maintenanceG <- Blanks_Main_lumped1 %>%
  filter(Blank.type != "Flake") %>% #check only laminar
  filter(!Platform_lump %in% c("No butt", "No Prox part", "ND", "NA", "Crushed", "Concave", "Natural", "Other")) %>% #removing blanks without clear info
  filter(!Abrasion.simpl %in% c("NA", "ND")) %>% #removing blanks without clear info
  filter(Technology.Phase %in% c("MGMT", "Ordinary")) %>% #getting only Ordinary
  select(SiteBlank.typeTech, Abrasion.simpl, Platform_lump) %>%
  na.omit()
...
fviz_screeplot(res.mca_Platform, addlabels = TRUE, ylim = c(0, 45)) #To visualize the percentages of inertia explained by each MCA dimensions. Dim1 and Dim2 explain 58.136% of the variance
...
fviz_contrib(res.mca_Platform, choice = "var", axes = 1, top = 15) #contribution to Dim 1. above the threshold are  Abrasion.simpl_No, Natural

fviz_contrib(res.mca_Platform, choice = "var", axes = 2, top = 15) #contribution to Dim 2. above the threshold is other, Plain

MCA_Platform_plot <- fviz_mca_var(
  res.mca_Platform,
  col.var = "contrib",
  label = "all",
  invisible = "none",
...
MCA_Platform_plot
ggsave("MCAPlatformALL.tiff", plot = MCA_Platform_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
ggsave("MCAPlatformALL.png", plot = MCA_Platform_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)    


MCA_Platform_ind <- fviz_mca_ind(
  res.mca_Platform, 
  label = "none",                   # hide individual labels
...
ggsave("MCAPlatformALLind.tiff", plot = MCA_Platform_ind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
ggsave("MCAPlatformALLind.png", plot = MCA_Platform_ind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
```
...
# Select relevant columns and remove missing values
platform_maintenanceGC <- Blank_Main_Control_elon %>%
  filter(!Platform_lump %in% c("No butt", "No prox part", "No Prox part", "ND", "NA", "Other", "Crushed", "Concave", "Cortical", "Natural")) %>% #removing blanks without clear info
  select(SiteBlank.type, Platform_lump) %>%
  na.omit()
  
summary(platform_maintenanceGC)

# Run MCA
res.mca_PlatformC <- MCA(platform_maintenanceGC, graph = FALSE, quali.sup =1, method = "Burt") #Site, SiteBlank.type, SiteBlank.typeTech are not part of the active variables used to compute the MCA but is considered as supplementary information. The MCA analysis is performed based on the active variables in platform_maintenanceG, and the supplementary variables are used to provide additional information or context to the analysis. Their coordinates are predicted using only the information provided by the performed multiple correspondence analysis on active variables/individuals.

print(res.mca_PlatformC)
fviz_screeplot(res.mca_PlatformC, addlabels = TRUE, ylim = c(0, 45)) #To visualize the percentages of inertia explained by each MCA dimensions. Dim1 and Dim2 explain 40.000% of the variance
summary(res.mca_PlatformC)


fviz_mca_var(res.mca_PlatformC, choice = "mca.cor", 
            repel = TRUE, 
            ggtheme = theme_minimal()) #checking to which dimensions the variable are correlated
fviz_mca_var(res.mca_PlatformC, 
             repel = TRUE, # 
             ggtheme = theme_minimal()) #Variable categories with a similar profile are grouped together. Negatively correlated variable categories are positioned on opposite sides of the plot origin (opposed quadrants).The distance between category points and the origin measures the quality of the variable category on the factor map. Category points that are away from the origin are well represented on the factor map.

var <- get_mca_var(res.mca_PlatformC)
head(var$cos2, 20) #If a variable category is well represented by two dimensions, the sum of the cos2 is closed to one. For some of the row items, more than 2 dimensions are required to perfectly represent the data.


fviz_cos2(res.mca_PlatformC, choice = "var", axes = 1:2) #visualising

fviz_contrib(res.mca_PlatformC, choice = "var", axes = 1, top = 15) #contribution to Dim 1. above the threshold are  Abrasion.simpl_No, Natural

fviz_contrib(res.mca_PlatformC, choice = "var", axes = 2, top = 15) #contribution to Dim 2. above the threshold is other, Plain

MCA_PlatformC_plot <- fviz_mca_var(
  res.mca_PlatformC,
  col.var = "contrib",
  label = "all",
  invisible = "none",
...
MCA_PlatformC_plot
ggsave("MCAPlatformCONTROL.tiff", plot = MCA_PlatformC_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
ggsave("MCAPlatformCONTROL.png", plot = MCA_PlatformC_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)

MCA_PlatformC_ind <- fviz_mca_ind(
  res.mca_PlatformC, 
  label = "none",                   # hide individual labels
  habillage = "SiteBlank.type", # color by groups
  addEllipses = TRUE,               # add confidence ellipses
...
ggsave("MCAPlatformCONTROLind.tiff", plot = MCA_PlatformC_ind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
ggsave("MCAPlatformCONTROLind.png", plot = MCA_PlatformC_ind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 
```
...
SiteType <- as.data.frame(platform_maintenanceGO$SiteBlank.type)

mca.PlatformO <- MCA_PlatformO %>%
  select(1:2)

Mca.PlatformO <- bind_cols(SiteType, mca.PlatformO)

Mca.PlatformO <- Mca.PlatformO %>%
  mutate(SiteType = platform_maintenanceGO$SiteBlank.type, "dim1" = coord.Dim.1, "dim2" =  coord.Dim.2)%>%
...
unique(Mca.PlatformO$SiteType)

#setting the unique comparisons 
assemblages <- c("A.b", "A.B", "R.b", "R.B", "C.b", "C.B", "F.B", "F.b")
...
comparisons_PlatformO <- c(
  "A.b-A.B", "A.b-R.b", "A.b-R.B", "A.b-C.b",
  "A.b-C.B", "A.b-F.B", "A.b-F.b", "A.B-R.b",
  "A.B-R.B", "A.B-C.b", "A.B-C.B", "A.B-F.B",
  "A.B-F.b", "R.b-R.B", "R.b-C.b", "R.b-C.B",
  "R.b-F.B", "R.b-F.b", "R.B-C.b", "R.B-C.B",
  "R.B-F.B", "R.B-F.b", "C.b-C.B", "C.b-F.B",
  "C.b-F.b", "C.B-F.B", "C.B-F.b", "F.B-F.b"
)
...
boot_dim1 <- mcppb20(dim1 ~ SiteType + SiteType^2, data = Mca.PlatformO, nboot = 5000)
boot_dim2 <- mcppb20(dim2 ~ SiteType, data = Mca.PlatformO, nboot = 5000)
...
multi_comp_dim1$SiteType <- comparisons_PlatformO
...
labels_dim1 <- setNames(multi_comp$`p-valuedim1`, multi_comp_dim1$SiteType)
labels_dim2 <- setNames(multi_comp$`p-valuedim2`, multi_comp_dim2$SiteType)
...
ggsave("ANOVAPlatformOrdinaryNew.tiff", plot = ANOVAPlatformO, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 
ggsave("ANOVAPlatformOrdinaryNew.png", plot = ANOVAPlatformO, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)

ANOVAPlatformOMCA <- grid.arrange(ANOVAPlatformO, MCA_PlatformO_plot, ncol = 1)

ggsave("ANOVAMCAcombinedPlatONew.tiff", plot = ANOVAPlatformOMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300) 
ggsave("ANOVAMCAcombinedPlatONew.png", plot = ANOVAPlatformOMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300)
...
ANOVAPlatformMMCA <- grid.arrange(ANOVAPlatformM, MCA_PlatformM_plot, ncol = 1)

ggsave("ANOVAMCAcombinedPlatM.tiff", plot = ANOVAPlatformMMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300) 
ggsave("ANOVAMCAcombinedPlatM.png", plot = ANOVAPlatformMMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300)
...
```{r}
library(dplyr)
MCA_Platform <- as.data.frame(res.mca_Platform$ind)
SiteTypeTech <- as.data.frame(platform_maintenanceG$SiteBlank.typeTech)

mca.Platform <- MCA_Platform %>%
  select(1:2)

Mca.Platform <- bind_cols(SiteTypeTech, mca.Platform)

Mca.Platform <- Mca.Platform %>%
  mutate(SiteTypeTech = platform_maintenanceG$SiteBlank.typeTech, "dim1" = coord.Dim.1, "dim2" =  coord.Dim.2)%>%
  select(4:6) #creating a dataframe with the Dim1 and Dim2 coordinates obtained with MCA_Reduction of each individual (blank)

###create unique comparisons
unique(Mca.Platform$SiteTypeTech)

#setting the unique comparisons 
assemblages <- c("A.b.M", "A.b.O", "A.B.M","A.B.O", "R.b.M", "R.b.O", "R.B.M", "R.B.O", "C.b.M", "C.b.O", "C.B.M","C.B.O", "F.B.M", "F.B.O", "F.b.M", "F.b.O")
...
"A.b.M-A.b.O" ,"A.B.M-A.b.O" ,"R.b.M-A.b.O" ,"R.B.M-A.b.O", "C.b.M-A.b.O", "C.B.M-A.b.O" ,"F.B.M-A.b.O" ,"F.b.M-A.b.O", "A.b.M-A.B.O","A.B.M-A.B.O" ,"R.b.M-A.B.O", "R.B.M-A.B.O", "C.b.M-A.B.O", "C.B.M-A.B.O", "F.B.M-A.B.O", "F.b.M-A.B.O", "A.b.M-R.b.O", "A.B.M-R.b.O","R.b.M-R.b.O", "R.B.M-R.b.O" ,"C.b.M-R.b.O", "C.B.M-R.b.O" ,"F.B.M-R.b.O", "F.b.M-R.b.O", "A.b.M-R.B.O", "A.B.M-R.B.O" ,"R.b.M-R.B.O","R.B.M-R.B.O", "C.b.M-R.B.O", "C.B.M-R.B.O", "F.B.M-R.B.O,", "F.b.M-R.B.O", "A.b.M-C.b.O", "A.B.M-C.b.O" ,"R.b.M-C.b.O" ,"R.B.M-C.b.O","C.b.M-C.b.O" ,"C.B.M-C.b.O", "F.B.M-C.b.O", "F.b.M-C.b.O", "A.b.M-C.B.O" ,"A.B.M-C.B.O", "R.b.M-C.B.O", "R.B.M-C.B.O", "C.b.M-C.B.O","C.B.M-C.B.O" ,"F.B.M-C.B.O", "F.b.M-C.B.O", "A.b.M-F.B.O", "A.B.M-F.B.O", "R.b.M-F.B.O" ,"R.B.M-F.B.O", "C.b.M-F.B.O" ,"C.B.M-F.B.O","F.B.M-F.B.O", "F.b.M-F.B.O", "A.b.M-F.b.O", "A.B.M-F.b.O", "R.b.M-F.b.O", "R.B.M-F.b.O", "C.b.M-F.b.O", "C.B.M-F.b.O" ,"F.B.M-F.b.O", "F.b.M-F.b.O"
)
...
boot_dim1 <- mcppb20(dim1 ~ SiteTypeTech, data = Mca.Platform, nboot = 5000)
boot_dim2 <- mcppb20(dim2 ~ SiteTypeTech, data = Mca.Platform, nboot = 5000)
...
The Convexity domain is comprising the Variables Axiality.simpl, Outline.morphology, Symmetry, Cross.section, Profile, Torsion, Distal.end.morpho  to check if there's association between morphological attributes attributed to a particular position on the core volume.
...
# Modify the theme settings for each plot
Gen.Ax <- Gen.Ax + theme(plot.margin = margin(10, 10, 20, 20))
Gen.Torsion <- Gen.Torsion + theme(plot.margin = margin(10, 10, 20, 20))
Gen.Profile <- Gen.Profile + theme(plot.margin = margin(10, 10, 20, 20))
Gen.Out.Morph <- Gen.Out.Morph + theme(plot.margin = margin(10, 10, 20, 20))
Gen.Symmetry <- Gen.Symmetry + theme(plot.margin = margin(10, 10, 20, 20))
Gen.Cross.Section <- Gen.Cross.Section + theme(plot.margin = margin(10, 10, 20, 20))
Gen.Distal.end.morpho <- Gen.Distal.end.morpho + theme(plot.margin = margin(10, 10, 20, 20))

# Arrange the plots with modified theme settings
ConvexityDom <- ggarrange(
  Gen.Ax, Gen.Torsion, Gen.Profile, Gen.Out.Morph, 
  Gen.Symmetry, Gen.Cross.Section, Gen.Distal.end.morpho,  
  labels = c("Axiality","Profile", "Outline", "Torsion", "Symmetry", "Cross", "Distal"), 
  ncol = 2, nrow = 4
)
# Save the combined plot
ggsave("ConvexityDom.tiff", plot = ConvexityDom, width = 66.96604, height = 32.75542, units = "cm", dpi = 300)
```
...
# Select relevant columns and remove missing values
convexity_maintenanceGO <- Blanks_Main_lumped1 %>%
  filter(Blank.type != "Flake") %>% #check only laminar
  ...
  select(SiteBlank.typeTech, Blank.type, Axiality.simpl, Outline.morphology, Symmetry, Cross.section, Profile, Torsion, Distal.end.morpho) %>%
  na.omit()
...
fviz_screeplot(res.mca_ConvexityO, addlabels = TRUE, ylim = c(0, 45)) #To visualize the percentages of inertia explained by each MCA dimensions. Dim1 and Dim2 explain 44.132% of the variance
...
MCA_ConvexityO_plot <- fviz_mca_var(res.mca_ConvexityO, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
             repel = TRUE, # avoid text overlapping (slow)
             ggtheme = theme_classic()
             )
MCA_ConvexityO_plot
ggsave("MCAConvexityOrdinaryNew.tiff", plot = MCA_ConvexityO_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
ggsave("MCAConvexityOrdinaryNew.png", plot = MCA_ConvexityO_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 


MCA_ConvexityOind <- fviz_mca_ind(res.mca_ConvexityO, 
             label = "none", # hide individual labels
             habillage = "SiteBlank.typeTech", # color by groups
             addEllipses = TRUE, 
...
ggsave("MCA_ConvexityOind.tiff", plot = MCA_ConvexityOind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
ggsave("MCA_ConvexityOind.png", plot = MCA_ConvexityOind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 
```
####Management
```{r}
library(FactoMineR)
...
MCA_ConvexityM_plot <- fviz_mca_var(res.mca_ConvexityM, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
             repel = TRUE, # avoid text overlapping (slow)
             ggtheme = theme_classic()
             )
MCA_ConvexityM_plot
ggsave("MCAConvexityMGMT.tiff", plot = MCA_ConvexityM_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
ggsave("MCAConvexityMGMT.png", plot = MCA_ConvexityM_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 


MCA_ConvexityMind <- fviz_mca_ind(res.mca_ConvexityM, 
             label = "none", # hide individual labels
             habillage = "SiteBlank.typeTech", # color by groups
             addEllipses = TRUE, 
             ellipse.type = "confidence",
...
ggsave("MCA_ConvexityMind.tiff", plot = MCA_ConvexityMind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
ggsave("MCA_ConvexityMind.png", plot = MCA_ConvexityMind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 
```
####ALL
```{r}
library(FactoMineR)
...
# Select relevant columns and remove missing values
convexity_maintenanceG <- Blanks_Main_lumped1 %>%
  filter(Blank.type != "Flake") %>% #check only laminar
  filter(!Axiality.simpl %in% c("NA", "ND")) %>% #removing blanks without clear info
  filter(!Symmetry %in% c("NA", "ND")) %>% #removing blanks without clear info
  filter(!Outline.morphology %in% c("NA", "ND")) %>% #removing blanks without clear info
  ...
  filter(!Torsion %in% c("NA", "ND")) %>% #removing blanks without clear info
  filter(!Distal.end.morpho %in% c("NA", "ND")) %>% #removing blanks without clear info
  filter(Technology.Phase %in% c("Ordinary", "MGMT")) %>% #only ordinary
  select(SiteBlank.typeTech, Blank.type, Axiality.simpl, Outline.morphology, Symmetry, Cross.section, Profile, Torsion, Distal.end.morpho) %>%
  na.omit()
...
# Run MCA
res.mca_Convexity <- MCA(convexity_maintenanceG, graph = FALSE, quali.sup =1:2, method = "Burt") #Site, SiteBlank.type, SiteBlank.typeTech are not part of the active variables used to compute the MCA but is considered as supplementary information. The MCA analysis is performed based on the active variables in platform_maintenanceG, and the supplementary variables are used to provide additional information or context to the analysis. Their coordinates are predicted using only the information provided by the performed multiple correspondence analysis on active variables/individuals.

print(res.mca_Convexity)
fviz_screeplot(res.mca_Convexity, addlabels = TRUE, ylim = c(0, 45)) #To visualize the percentages of inertia explained by each MCA dimensions. Dim1 and Dim2 explain 45.865% of the variance
...
MCA_Convexity_plot <- fviz_mca_var(res.mca_Convexity, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
             repel = TRUE, # avoid text overlapping (slow)
             ggtheme = theme_classic()
             )
MCA_Convexity_plot
ggsave("MCAConvexityALL.tiff", plot = MCA_Convexity_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
ggsave("MCAConvexityALL.png", plot = MCA_Convexity_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 


MCA_Convexityind <- fviz_mca_ind(res.mca_Convexity, 
             label = "none", # hide individual labels
             habillage = "SiteBlank.typeTech", # color by groups
             addEllipses = TRUE, 
             ellipse.type = "confidence",
...
ggsave("MCA_ConvexityALLind.tiff", plot = MCA_Convexityind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
ggsave("MCA_ConvexityALLind.png", plot = MCA_Convexityind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 
```
###only blade bladelet info
```{r}
library(FactoMineR)
library(factoextra)
library(dplyr)
...
# Select relevant columns and remove missing values
convexity_maintenanceG1 <- Blanks_Main_lumped1 %>%
  filter(Blank.type != "Flake") %>% #check only laminar
  filter(!Axiality.simpl %in% c("NA", "ND")) %>% #removing blanks without clear info
  filter(!Symmetry %in% c("NA", "ND")) %>% #removing blanks without clear info
  filter(!Outline.morphology %in% c("NA", "ND")) %>% #removing blanks without clear info
  ...
  filter(!Distal.end.morpho %in% c("NA", "ND")) %>% #removing blanks without clear info
  filter(Technology.Phase %in% c("Ordinary", "MGMT")) %>% #only ordinary
  select(Blank.type,SiteBlank.type, Axiality.simpl, Outline.morphology, Symmetry, Cross.section, Profile, Torsion, Distal.end.morpho) %>%
  na.omit()
  
summary(convexity_maintenanceG)

# Run MCA
res.mca_Convexity1 <- MCA(convexity_maintenanceG1, graph = FALSE, quali.sup =1:2, method = "Burt") #Site, SiteBlank.type, SiteBlank.typeTech are not part of the active variables used to compute the MCA but is considered as supplementary information. The MCA analysis is performed based on the active variables in platform_maintenanceG, and the supplementary variables are used to provide additional information or context to the analysis. Their coordinates are predicted using only the information provided by the performed multiple correspondence analysis on active variables/individuals.

print(res.mca_Convexity1)
fviz_screeplot(res.mca_Convexity, addlabels = TRUE, ylim = c(0, 45)) #To visualize the percentages of inertia explained by each MCA dimensions. Dim1 and Dim2 explain 45.865% of the variance
summary(res.mca_Convexity1)

fviz_mca_var(res.mca_Convexity1, choice = "mca.cor", 
            repel = TRUE, 
            ggtheme = theme_minimal()) #checking to which dimensions the variable are correlated
fviz_mca_var(res.mca_Convexity1, 
             repel = TRUE, # 
             ggtheme = theme_minimal()) #Variable categories with a similar profile are grouped together. Negatively correlated variable categories are positioned on opposite sides of the plot origin (opposed quadrants).The distance between category points and the origin measures the quality of the variable category on the factor map. Category points that are away from the origin are well represented on the factor map.

var <- get_mca_var(res.mca_Convexity1)
head(var$cos2, 50) #If a variable category is well represented by two dimensions, the sum of the cos2 is closed to one. For some of the row items, more than 2 dimensions are required to perfectly represent the data.

fviz_cos2(res.mca_Convexity1, choice = "var", axes = 1:2) #visualising

fviz_contrib(res.mca_Convexity1, choice = "var", axes = 1, top = 15) #contribution to Dim 1. 

fviz_contrib(res.mca_Convexity1, choice = "var", axes = 2, top = 15) #contribution to Dim 2. 

MCA_Convexity_plot1 <- fviz_mca_var(res.mca_Convexity1, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
             repel = TRUE, # avoid text overlapping (slow)
             ggtheme = theme_classic()
             )
MCA_Convexity_plot1
ggsave("MCAConvexityALLnotech.tiff", plot = MCA_Convexity_plot1, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
ggsave("MCAConvexityALLnotech.png", plot = MCA_Convexity_plot1, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 


MCA_Convexityind1 <- fviz_mca_ind(res.mca_Convexity1, 
             label = "none", # hide individual labels
             habillage = "SiteBlank.type", # color by groups
             addEllipses = TRUE, 
             ellipse.type = "confidence",
...
ggsave("MCA_ConvexityALLindnotech.tiff", plot = MCA_Convexityind1, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
ggsave("MCA_ConvexityALLindnotech.png", plot = MCA_Convexityind1, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
```
...
convexity_maintenanceC <- Blank_Main_Control_elon %>%
  filter(!Outline.morphology %in% c("NA", "ND", "Other", "No Dist Part")) %>% #removing blanks without clear info
  filter(!Profile %in% c("NA", "ND", "Other", "Irregular")) %>% #removing blanks without clear info
  filter(!Distal.end.morpho %in% c("NA", "ND", "Other", "No Dist Part")) %>% #removing blanks without clear info
  select(SiteBlank.type, Blank.type, Outline.morphology, Cross.section, Profile, Distal.end.morpho) %>%
  na.omit()
  
summary(convexity_maintenanceC)

# Run MCA
res.mca_ConvexityC <- MCA(convexity_maintenanceC, graph = FALSE, quali.sup =1:2, method = "Burt") #Site, SiteBlank.type, SiteBlank.typeTech are not part of the active variables used to compute the MCA but is considered as supplementary information. The MCA analysis is performed based on the active variables in platform_maintenanceG, and the supplementary variables are used to provide additional information or context to the analysis. Their coordinates are predicted using only the information provided by the performed multiple correspondence analysis on active variables/individuals.

print(res.mca_ConvexityC)
fviz_screeplot(res.mca_ConvexityC, addlabels = TRUE, ylim = c(0, 45)) #To visualize the percentages of inertia explained by each MCA dimensions. Dim1 and Dim2 explain 33.269% of the variance
summary(res.mca_ConvexityC)

fviz_mca_var(res.mca_ConvexityC, choice = "mca.cor", 
            repel = TRUE, 
            ggtheme = theme_minimal()) #checking to which dimensions the variable are correlated
fviz_mca_var(res.mca_ConvexityC, 
             repel = TRUE, # 
             ggtheme = theme_minimal()) #Variable categories with a similar profile are grouped together. Negatively correlated variable categories are positioned on opposite sides of the plot origin (opposed quadrants).The distance between category points and the origin measures the quality of the variable category on the factor map. Category points that are away from the origin are well represented on the factor map.

var <- get_mca_var(res.mca_ConvexityC)
head(var$cos2, 50) #If a variable category is well represented by two dimensions, the sum of the cos2 is closed to one. For some of the row items, more than 2 dimensions are required to perfectly represent the data.

fviz_cos2(res.mca_ConvexityC, choice = "var", axes = 1:2) #visualising

fviz_contrib(res.mca_ConvexityC, choice = "var", axes = 1, top = 15) #contribution to Dim 1. 

fviz_contrib(res.mca_ConvexityC, choice = "var", axes = 2, top = 15) #contribution to Dim 2. 

MCA_ConvexityC_plot <- fviz_mca_var(res.mca_ConvexityC, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
             repel = TRUE, # avoid text overlapping (slow)
             ggtheme = theme_classic()
             )
MCA_ConvexityC_plot
ggsave("MCAConvexityCONTROL.tiff", plot = MCA_ConvexityC_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
ggsave("MCAConvexityCONTROL.png", plot = MCA_ConvexityC_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 


MCA_ConvexityCind <- fviz_mca_ind(res.mca_ConvexityC, 
             label = "none", # hide individual labels
             habillage = "SiteBlank.type", # color by groups
             addEllipses = TRUE, 
             ellipse.type = "confidence",
...
ggsave("MCA_ConvexityCONTROLind.tiff", plot = MCA_ConvexityCind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)   
ggsave("MCA_ConvexityCONTROLind.png", plot = MCA_ConvexityCind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 
```
...
ANOVAConvexityOMCA <- grid.arrange(ANOVAConvexityO, MCA_ConvexityO, ncol = 1)

ggsave("ANOVAMCAcombinedConvO.tiff", plot = ANOVAConvexityOMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300) 
ggsave("ANOVAMCAcombinedConvO.png", plot = ANOVAConvexityOMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300)
...
ANOVAConvexityMMCA <- grid.arrange(ANOVAConvexityM, MCA_ConvexityM_plot, ncol = 1)

ggsave("ANOVAMCAcombinedConvM.tiff", plot = ANOVAConvexityMMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300) 
ggsave("ANOVAMCAcombinedConvM.png", plot = ANOVAConvexityMMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300)
...
MCA_ReductionO_plot <- fviz_mca_var(res.mca_ReductionO, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
             repel = TRUE, # avoid text overlapping (slow)
             ggtheme = theme_classic()
             )#
MCA_ReductionO_plot
ggsave("MCAReductionOrdinary.tiff", plot = MCA_ReductionO_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 
ggsave("MCAReductionOrdinary.png", plot = MCA_ReductionO_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)


MCA_ReductionOind <- fviz_mca_ind(res.mca_ReductionO, 
             label = "none", # hide individual labels
             habillage = "SiteBlank.typeTech", # color by groups
...
ggsave("MCAReductionOrdinaryind.tiff", plot = MCA_ReductionOind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 
ggsave("MCAReductionOrdinaryind.png", plot = MCA_ReductionOind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
...
MCA_ReductionM_plot <- fviz_mca_var(res.mca_ReductionM, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
             repel = TRUE, # avoid text overlapping (slow)
             ggtheme = theme_classic()
             )#
MCA_ReductionM_plot
ggsave("MCAReductionMGMT.tiff", plot = MCA_ReductionM_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 
ggsave("MCAReductionMGMT.png", plot = MCA_ReductionM_plot, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)

MCA_ReductionMind <- fviz_mca_ind(res.mca_ReductionM, 
             label = "none", # hide individual labels
             habillage = "SiteBlank.typeTech", # color by groups
             addEllipses = TRUE, ellipse.type = "confidence",
             ggtheme = theme_classic(),
...
ggsave("MCAReductionMGMTind.tiff", plot = MCA_ReductionMind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300) 
ggsave("MCAReductionMGMTind.png", plot = MCA_ReductionMind, width = 43.656255, height = 20.29354, units = "cm", dpi = 300)
...
ANOVAReductionOMCA <- grid.arrange(ANOVAReductionO, MCA_ReductionO_plot, ncol = 1)

ggsave("ANOVAMCAcombinedRedO.tiff", plot = ANOVAReductionOMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300) 
ggsave("ANOVAMCAcombinedRedO.png", plot = ANOVAReductionOMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300)
...
ANOVAReductionMMCA <- grid.arrange(ANOVAReductionM, MCA_ReductionM_plot, ncol = 1)

ggsave("ANOVAMCAcombinedRedM.tiff", plot = ANOVAReductionMMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300) 
ggsave("ANOVAMCAcombinedRedM.png", plot = ANOVAReductionMMCA, width = 66.96604, height = 32.75542, units = "cm", dpi = 300)
