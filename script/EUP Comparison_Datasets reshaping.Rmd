...
This is an R Markdown document containing the Rcode and workflow for the Tracking the emergence of the Upper Palaeolithic in western Asia and Europe: a Multiple Correspondence Analysis of Protoaurignacian and southern Ahmarian lithics article. This section is dedicated to datasets merging and reshaping
...
|                       |                        |                                                            | Technocomplex        | main Technocomplex                                                                                                     |
...
| Rloc                  | Rloc                   | Rloc                                                       | Retouch Location     | retouch localisation on the artefact                                                                                   |
| Rdis                  | Rdis                   | Rdis                                                       | Retouch Distribution | retouch extent on the artefact                                                                                         |
...
Gennai_ROM_ANS_Blanks_reshaped <- Gennai_Rom_Ansab_Blanks %>%
  ...
  mutate(Site = case_when(
      ...
      Site == "Ansab" ~ "AN")) %>% 
  mutate(Technocomplex = Site) %>% 
  mutate(Technocomplex = case_when(
      Technocomplex == "AN" ~ "S.Ahm", 
      Technocomplex == "ROM" ~ "Proto"))%>%
...
mutate(Technology.Ext= case_when(
 ...
 Gennai_ROM_ANS_Blanks_reshaped <- Gennai_ROM_ANS_Blanks_reshaped %>% 
   select(Site, Technocomplex, Layer, ID, Preservation, Cortex.y.n, Cortex, Cortex.position, Length, Width, Thickness, Elongation, Robustness, Platform, Bulb, Lip, Abrasion, Axiality, Outline.morphology, Symmetry, Cross.section, Profile, Torsion, Distal.end.morpho, Blank.type1, Blank.type2, Technology.Phase, Technology.Ext, Number.negatives, Negatives.type, Dorsal.scar.1, Dorsal.scar.2, Tool, Retouch.Position, Retouch.Location, Retouch.Distribution, Typology)
...
Blanks_Main_merged <- Blanks_Main_merged %>%
  mutate(Typology= case_when(
 Typology == "Burin double" ~ "Burin",
 Typology == "Grattoir" ~ "Endscraper",  
 Typology == ("Lame retouchée") ~ "Blade retouched",
 Typology == ("Lamelle retouchée") ~ "Bladelet retouched",
 Typology == "Lame à dos" ~ "Backed blade",
 Typology %in% c("lateral retouched bladelet", "Lamelle retouchée latéralement ?", "Lamelle retouchée latéralement") ~ "Bladelet with lateral retouch",
 Typology %in% c("lateral retouched blade", "Lame retouchée latéralement") ~ "Blade with lateral retouch",
TRUE ~ Typology
 )) %>%
 mutate(Retouch.Distribution= case_when(
 Retouch.Distribution  %in% c("Discontinous", "Discontinuous") ~ "Partial",
 Retouch.Distribution == "Continuous + Discontinous" ~ "Continuous + Partial",
 TRUE ~ Retouch.Distribution
 )) %>%
mutate(Retouch.Location= case_when(
 Retouch.Location == "bilateral" ~ "Bilateral",
 TRUE ~ Retouch.Location))
 


write_xlsx(Blanks_Main_merged, "Ans_Rom_Fum_CTS_Blanks_merged.xlsx")
```
##Control Solutrean Dataset
```{r}
Solutrean_Blanks <- read_excel(file.choose()) #Get the Cascalheira_Blanks.xlsx
View(Solutrean_Blanks)
head(Solutrean_Blanks)

Solutrean_Laminar_Blanks_reshaped <- Solutrean_Blanks %>%
  rename(Site = "Context", 
         Cortex="CortexPerc", 
         Cortex.position = "CortexLoc", 
         Platform = "PlatformType", 
         Outline.morphology = "EdgeShape", 
         Cross.section = "CrossSection", 
         Distal.end.morpho = "Termination",
         Blank.type1 = "Class", 
         Number.negatives = "ScarNumber", 
         Dorsal.scar.2 = "DorsalPattern") %>%
  mutate(
    Thickness = if_else(is.na(as.numeric(Thickness)), NA, as.numeric(Thickness)),
    Length = as.numeric(Length),
    Width = as.numeric(Width)
  ) %>%
  filter(Blank.type1 != "Flake") %>%
  mutate(Blank.type2 = Blank.type1) %>%
  mutate(Blank.type1 = if_else(Blank.type1 == "Elongated product", "Laminar", Blank.type1)) %>%
  mutate(Blank.type2 = if_else(Width < 12, "Bladelet", "Blade"))%>%
  mutate(Technocomplex = "Solu")%>%
  mutate(Elongation = Length / Width) %>%
  mutate(Robustness = Width / Thickness) %>%
  mutate(Outline.morphology = case_when(
    Outline.morphology %in% c("Divergent", "Biconvex", "Parallel", "Convergent") ~ Outline.morphology,
    Outline.morphology == "Irregular" ~ "ND",
    Outline.morphology == "Other" ~ "ND",
    Outline.morphology == "Circular" ~ "ND",
    Outline.morphology == "Déjeté" ~ "Off-axis"
  )) %>%
  mutate(Platform =case_when(
    Platform == "Dihedral"~ "Facetted",
    Platform == "Faceted"~ "Facetted", 
    TRUE ~ Platform))%>% # Platform
  mutate(Cortex = case_when(
    Cortex %in% c("0") ~ "No Cortex", 
    Cortex %in% c("<25%", "25-75%") ~ "Semi",
    Cortex == "75-95%" ~ "Extensive",
    Cortex %in% c(">95%", "1") ~ "Full")) %>%
  mutate(Cortex.y.n = Cortex, .after = Cortex.position) %>% #INSERT CORTEX.Y.N
  mutate(Cortex.y.n =case_when(Cortex.y.n == "No Cortex" ~ "No", 
                              Cortex.y.n %in% c("Extensive",
                                               "Full",
                                               "Semi") ~"Yes"))%>% #Cortex.y.n
  mutate(Profile = case_when(
    Profile %in% c("Straight", "Curved", "Twisted") ~ Profile,
    Profile == "Irregular" ~ "ND",
    TRUE ~ NA_character_
  )) %>%
  mutate(Torsion = if_else(Profile == "Twisted", "Yes", "No"))%>%
  mutate(Axiality = if_else(Outline.morphology == "Off-axis", "Off-axis", "On-axis")) %>%
  mutate(Cross.section = case_when(
    Cross.section == "Trapezoidal" ~ "Polyhedric",
    Cross.section %in% c("Irregular", "Lenticular", "Other") ~ "ND",
  TRUE ~ Cross.section))%>%
  mutate(Distal.end.morpho = case_when(
    Distal.end.morpho == "Hinged" ~ "ND",
   Distal.end.morpho == "Pointed" ~ "Feathered",
   Distal.end.morpho == "Overpassed" ~ "Plunging",
  TRUE ~ Distal.end.morpho)) %>%
mutate(Dorsal.scar.1 = Dorsal.scar.2, .after = Dorsal.scar.2) %>% # insert Dorsal.scar.1
mutate(Dorsal.scar.1 =case_when(
    Dorsal.scar.1 == "Unidentifiable"~ "ND",
    Dorsal.scar.1 == "Perpendicular"~ "Orthogonal", 
    TRUE ~ Dorsal.scar.1)) # Dorsal.scar.1
  
library(writexl)

# Write data frame to Excel file
write_xlsx(Solutrean_Laminar_Blanks_reshaped, "Solutrean_Laminar_Blanks_reshaped.xlsx")
...
```{r}
# Subset Blanks_Main_merged
Blanks_Main_merged_subset <- Blanks_Main_merged[, c("Site", "Blank.type1", "Distal.end.morpho", "Outline.morphology", "Length",
                                                     "Cortex", "Thickness", "Width", "Cortex.position", "Cortex.y.n",
                                                     "Number.negatives", "Profile", "Cross.section", "Platform",
                                                     "Dorsal.scar.2", "Blank.type2", "Technocomplex", "Elongation",
                                                     "Robustness", "Torsion", "Axiality")]

# Subset Solutrean_Laminar_Blanks_reshaped
Solutrean_Laminar_Blanks_reshaped_subset <- Solutrean_Laminar_Blanks_reshaped[, c("Site", "Blank.type1", "Distal.end.morpho",
                                                                                   "Outline.morphology", "Length", "Cortex",
                                                                                   "Thickness", "Width", "Cortex.position",
                                                                                   "Cortex.y.n", "Number.negatives", "Profile",
                                                                                   "Cross.section", "Platform", "Dorsal.scar.2",
                                                                                   "Blank.type2", "Technocomplex", "Elongation",
                                                                                   "Robustness", "Torsion", "Axiality")]

# Combine the subsetted data frames
combinedControl_df <- rbind(Blanks_Main_merged_subset, Solutrean_Laminar_Blanks_reshaped_subset)

write_xlsx(combinedControl_df, "Control_Blanks.xlsx")
